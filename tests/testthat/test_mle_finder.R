...
test_that("Gradient formula works!",{
  ...
  a1 <- neg_gradient(beta =  beta1, design = design1, outcome = outcome1)
  a2 <- approx_grad(function(x) neg_log_likelihood(x, design = design1, outcome = outcome1, noise_var = 1), x = beta1, dx = .Machine$double.eps^(1/3))

  expect_true(are_all_close(
    a1, a2, abs_tol = 1e-4, rel_tol = 1e-4
  ))
})
